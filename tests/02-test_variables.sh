...
testNoVariables() {
	...
	assertEquals "Docker hub organization name (DOCKER_ORGANIZATION_NAME) not provided." "$check_variables_message"
}
...
testWithDOCKER_ORGANIZATION_NAME() {
	...
	check_variables_message=$(cat $TMP_FOLDER/testNoVariables)
	assertEquals "1" "$check_variables_status"
	assertEquals "Image maintainer (DOCKER_IMAGES_MAINTAINER) not provided." "$check_variables_message"
}
...
testWithoutDOCKER_REGISTRY_USER() {
	...
	check_variables_message=$(cat $TMP_FOLDER/testNoVariables)
	assertEquals "1" "$check_variables_status"
	assertEquals "Registry user (DOCKER_REGISTRY_USER) not provided." "$check_variables_message"
}
...
testWithoutDOCKER_REGISTRY_PASSWORD() {
	...
	check_variables_message=$(cat $TMP_FOLDER/testNoVariables)
	assertEquals "1" "$check_variables_status"
	assertEquals "Registry user password (DOCKER_REGISTRY_PASSWORD) not provided." "$check_variables_message"
}
